				...
				<table class="table data-table">
					...
					<tbody>
						...
						<tr id="row_{{ $emailtemplate->id }}">
							...
							<td class='name'>{{ ucwords($emailtemplate->template_type) }}</td>
							<td class='status'>
								@if($emailtemplate->template_type == 'admin')
								...
								{!! xss_clean(show_status(_lang('None'), 'secondary')) !!}
								@endif
								@endif
							</td>
							<td class="text-center">
								<a href="{{ route('admin.notification_templates.edit', $emailtemplate->id) }}" class="btn btn-primary btn-xs"><i class="ti-pencil-alt"></i>&nbsp;{{ _lang('Edit') }}</a>
							</td>
						</tr>
						@endforeach
					</tbody>
